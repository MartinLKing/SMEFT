...
\PMnote{These stages in SM-EFT are important for our discussion the model character of 
SM-EFT.
Therefore, let us point out at this stage some important difference between stage 1
(which is rather close to stage 2) and stage 3.} 

There is a significant difference in the \textbf{physicists' practice} towards finding BSM physics.
\PMnote{Nobody assumes that all dimension 6 operators of the SM-EFT simultaneously 
describe underlying physics. 
Physicists just take one or a few operators as relevant for BSM physics to solve what
they consider as potential problems or deviations.
This corresponds to the experience that is exemplified in the next Section.}  

\PMnote{Thus there is a significant difference between SM-EFT and selecting one operator in terms of 
\textbf{testability and target}.} 
Only an EFT whose operator structure represents an underlying physics can
make \PMnote{delete: successful} predictions. 
\PMnote{Is this true: note also stage 3 is NO SM-EFT anymore}
Such a prediction and an independent
test of the new vertex structure represented by the \PMnote{delete:SM-} EFT can only be
made in Stage~3.  
% Since the operators would still violate unitarity,
% it is clear that such a description cannot be extrapolated to
% arbitrary high energies, and hence
%we cannot assume that it is fundamental. 
However, the SM-EFT then
represents a target phenomenon and (may) imply new ones.  
\PMnote{(Leave this to later discussion: - Knuutila does not need representation)}
...
\PMnote{(Do we need this, anyway seems unclear:)} 
...
\PMnote{Discuss \textbf{representational} properties}
